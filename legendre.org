...
TODO NOTE in general case lagrangian may only be time dependent... but I think all this is complicated enough

The basic idea of Lagrangian is answering 'how does physics work for this system' if it has to go from state $(q_0, v_0)$ to $(q_1, v_1)$ within time interval $[t_0, t_q]$. TODO link to Baez?? To do that, we define a quantity called action:
$S = \int_{t_0}^{t_1} L(q(t), v(t)) dt$ and claim that the system chooses a 'path' (i.e. mapping $q \to (q(t), v(t))$), such that $S$ is stationary to first order.
